...
class Search extends React.Component {
  ...
  render() {
    ...
    return (
      <div>
        {this.props.is_open && (
          <div className="overlay" onClick={this.submit} />
        )}
        <div className={"sidebar " + openShow} id="mobile-search">
          <form className="sidebar-search" onSubmit={this.redirectSearch}>
            ...
            <button
              ...
              onClick={() => this.props.closeSide(false)}
            >
              {" "}
              Cancel{" "}
            </button>
          ...
          {filteredSneakers.slice(0, 5).map((sneaker) => (
            <ul className="search-items-container" onClick={this.submit}>
              <SearchItems sneaker={sneaker} submit={this.submit} />
            </ul>
          ))}
        </div>
      </div>
